...
namespace CustomLauncher
{
    ...
    class Launcher
    {
        ...
        public LauncherReturn Launch(string MaxMem, string JavaPath, string UserName, string VerName, int returnvalue = 0, bool dc = false, String ExtraMCArg = "", String ExtraJreArgs = "")
        {
            ...
            if (new MainForm().ini_Read(new MainForm().配置节, "OnlineMode") == "true")//如果要正版登录
            {
                auths = YggLogin(new MainForm().ini_Read(new MainForm().配置节, "Online-Email"), new Settings().EncryptStr(new MainForm().ini_Read(new MainForm().配置节, "OnlineMode-Password"), true)).rtvs;//调用YggLogin登录
            }
